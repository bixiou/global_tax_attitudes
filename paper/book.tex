% TODO: remplacer 4e de couv' (max 1700 signes espaces compris) par préface
% /!\ Dans les scénarios avec participation Centrale ou moins, l'Inde ne gagne pas grand chose (50G€/an dans Central)
% TODO: source ratio Dem USA and other TODO in book.R
% TODO? \textit{\$} => \$$_\text{PPA}$ \textit{\texteuro{}} => \euro{}$_\text{PPA}$ ? + "italique" "type perdrait"
...
% PNON compute carbon debt using price at t=0 and back-finding the price using the result that optimal carbon price grows with OR use Burke et al. (23), Robinson et al. (21) or Callaghan & Mankin (22) => plus tard
...
% Greta Thunberg, at COP24 (2018).

% Adresse TODO : À Greta, Rihanna, Obama et au Pape? À Mbappé? Benzema
...
Pour respecter ce budget carbone, les émissions de CO$_\text{2}$ (actuellement de 38~Gt par an) pourraient être réduite chaque année du même nombre de tonnes, jusqu'à atteindre zéro en 2077.  %Ce budget carbone pourrait être respecté en réduisant les émissions de CO$_\text{2}$ du même montant chaque année, en partant de leur valeur actuelle de 38 Gt jusqu'à zéro en 2077. 
...
Bien que la proportion d'humains vivant avec moins de 2~\textit{\texteuro{}} par jour ait été divisée par quatre dans les trente dernières années, l'extrême pauvreté concerne encore deux tiers de la population dans un pays comme le Malawi. En fait, avec l'augmentation de la population, il y a davantage d'Africains en situation d'extrême pauvreté aujourd'hui qu'il y a trente ans. Si l'extrême pauvreté s'est réduite durant la période, c'est uniquement grâce au développement de l'Asie, et en particulier de la Chine. % https://ourworldindata.org/poverty?insight=hundreds-of-millions-will-remain-in-extreme-poverty-on-current-trends#key-insights
...
Au total, la moitié des humains vit sous ce seuil de pauvreté\footnote{Cf. \href{https://ourworldindata.org/grapher/distribution-of-population-between-different-poverty-thresholds-up-to-30-dollars}{ourworldindata.org}.}. Combler l'écart qui les sépare de ce seuil coûterait 2~\% du PIB mondial en 2030\footnote{En parité de pouvoir d'achat, cet écart (le \textit{poverty gap}, qu'on peut traduire par \textit{l'étendue de la pauvreté}) est de 4~800 milliards de dollars, soit 3,4~\% du PIB mondial de 140~000 milliards. En faisant l'hypothèse d'une croissance mondiale annuelle de 3,5~\% (soit celle observée sur les 20 dernières années), on trouve %une estimation basse à 
...
son bien-être et celui de ses descendants. En effet, pour mettre fin au changement climatique, %--- dont les dégâts déjà supérieurs au coût de la décarbonation\footnote%{\citet{kotz_economic_2024}.} ---, TODO in Word
...
Dans un autre chapitre, \citet{cramton_international_2015} partent du principe que chaque État défend son intérêt national et proposent l'accord suivant entre les pays  volontaristes. % :maj %qui veulent agir contre le changement climatique
...
sur un plan qui doublerait le revenu moyen d'un pays comme le Burundi, en versant à chaque humain un revenu de base de 50~\euro{}/mois~? En février 2026, un groupe de 156 pays et 9 États états-uniens, couvrant 74~\% des émissions de CO$_\text{2}$ mondiales, signent un accord de principe pour négocier le plafonnement de leurs émissions de CO$_\text{2}$. S'ensuit une bataille de cinq longues années pour négocier ce plan, puis une implémentation laborieuse, qui prendra du retard, mais sera pleinement opérationnelle en 2037. Dopée par le revenu de base, l'Afrique connaît alors un boom économique sans précédent. Dans les années 2050, les États-Unis et la Russie finissent par rejoindre l'Union Climatique. L'Arabie Saoudite est le dernier pays à franchir le pas, en 2061. En 2084, le monde entier a atteint la neutralité carbone, et plus personne ne vit avec moins de 7~\textit{\texteuro{}} par jour. Ce n'était pas gagné~: la moitié de l'humanité vit sous ce seuil de pauvreté à l'heure où j'écris ces lignes. % 50% < 8$ according to World Bank's pretax (less if we use WID or posttax): 57% < 10$
...
le prix d'équilibre aura été découvert, de sorte que le prix sur le marché secondaire sera relativement prédictible et égal à celui des enchères. En fin d'année, les émissions issues des entreprises assujetties sont contrôlées, et ces entreprises doivent délivrer des permis à hauteur de leurs émissions. % les entreprises assujetties doivent alors restituer des permis d'émission à la même hauteur que leurs émissions constatées.
...
\footnote{Seuls les projets dont la séquestration est indiscutable seraient financés. Ainsi, la séquestration par gain de biomasse forestière ne serait financée que si les émissions liées à la perte de biomasse forestière sont tarifées par ailleurs. La séquestration du carbone pourrait être financée dès la première phase, là aussi à travers des taxes sur les plus riches. Sa valeur serait alors fixée au prix de marché, et la séquestration ainsi rémunérée viendrait gonfler d'autant le quota d'émissions mis aux enchères \citep{edenhofer_governance_2023}.}. % PS : après réflexion, je préfère le système où les crédits pour la séquestration carbone sont séparés du marché du carbone. Ils seraient rémunérés au prix de marché du carbone (et on pourrait également prévoir des subventions pour la R&D et des projets pilote). La séquestration rémunérée par l'UE pourrait venir gonfler d'autant le quota d'émissions mis aux enchères. Mais la séquestration devrait être rémunérée par d'autres ressources que les recettes de l'ETS (par exemple, un impôt sur la fortune). La raison de ma préférence pour cet arrangement est que l'intégralité des recettes liées aux émissions pourra être remise aux ménages (sous forme de transferts directs, de subventions et d'investissements) et le coût de la séquestration pourra reposer sur les plus riches (à travers un impôt affecté). Ça éviterait de se retrouver dans une situation ou une grosse part des recettes de l'ETS est captée par les entreprises de séquestration, et où on ne pourrait pas compenser les ménages pour les hausses de prix.
...
Plusieurs problèmes ont entaché Aadhaar \citep{dreze_aadhaar_2017}, mais on peut tirer des leçons de ces échecs pour ne pas les reproduire \citep{gelb_digital_2019,muralidharan_identity_2023}. En effet, les modalités qui fonctionnent sont désormais mieux connues. Par exemple, la meilleure pratique consiste à utiliser les protocoles open-source pour l'identification biométrique (\href{https://mosip.io}{MOSIP}) plutôt que des logiciels propriétaires.}, pour un coût d'à peine 1~\euro{} par personne enregistrée. Ce déploiement de l'identité biométrique est désormais répliqué ailleurs. % PNON Quand les projets en Afrique coûte en général entre 5 et 10~\euro{} par personne enregistrée, Aadhaar n'a coûté que 1\euro{} par personne\footnote{Cf. \cite{world_bank_state_2017}}.
...
% Vérifier l'identité grâce à une combinaison de méthodes, reposant sur une toile de confiance établie sans intervention d'agents.
% Inscription par internet, avec (+: obligatoire):
% + nom, prénom, lieu de résidence
% + numéro de téléphone (pouvant être partagé, dans ce cas indiquer lien familial ou autre)
% ? IMEI de l'appareil
% - email (institutionnel si possible)
% - pièce d'identité
% - compte en banque
% - compte réseau social
% + vidéo avec parole (et la pièce d'identité si la personne en a une)
% - localisation GPS
% => obtention d'un identifiant et d'un score de fiabilité (= unicité)
% - liens familiaux (identifiants des parents, enfants, frères et sœurs)
% - pour les jumeaux ou frères/sœurs qui se ressemblent, vidéo de la fratrie
% Lors d'un doute sur la fiabilité (par ex qq'un sans famille, avec qq'un qui lui ressemble dans une localité proche):
%   - localisation GPS activée en continu des comptes suspectés d'être une même personne
%   - suspension du versement du revenu de base
%   - convocation à un bureau pour prendre iris, photo (et empreintes?)
% Le revenu de base est versé sur le compte en banque enregistré ou à défaut, sur le numéro de tel. Pour en changer, le mot de passe + vidéo du visage sont requis.
% ENJEUX: confidentialité et protection des données; 
% => Pourquoi pas verser une monnaie de singe ? Ou mieux, ne payer que les conversions de la monnaie de singe en devises, pour réduire le coût du RdB?
...
% # 1960-2050   61   150    78   192<- Fanning & Hickel
...
. Pour amplifier le financement du développement, les banques multilatérales de développement doivent être recapitalisées et recevoir des Droits de Tirage Spéciaux~; la dette publique des pays à bas revenus doit être annulée ou restructurée~; % accélérer l'annulation de dettes publiques
...
Alors que je finalise ce livre, je découvre qu'une Initiative Citoyenne Européenne vient d'être déposée. %que le parti Volt vient de déposer une Initiative Citoyenne Européenne, 
...
passent peut-être à côté de meilleures voies pour atteindre la justice climatique, et je m'emploie d'ailleurs déjà à élaborer une nouvelle version du Plan avec des diplomates. % DONE un peu bancal entre redistr mondiale / justice climatique / CC 
...
Hélas, le Plan proposé ne traite pas ces problèmes. Il faut donc consulter d'autres travaux pour constituer un programme qui répondrait à tous les défis écologiques\footnote{Par exemple \citet{strassburg_reducing_2009,karsenty_geopolitique_2021} dans le cas des forêts.}. % PNON commenter la suite ?
...
% % Ce paragraphe ne soulève pas un problème propre à la taxation progressive, mais à l'absence de contrôle des frais professionnels (les abus existent même hors taxe carbone). Par ailleurs, il obscurcit le fait qu'idéalement les émissions d'une entreprise devrait être attribuées à (l'empreinte carbone de) ses clients.
...
Même si les pays exportateurs d'hydrocarbures perdront d'importantes ressources avec la décarbonation\footnote{Les pays les plus affectés seront les pays du Golfe. En particulier, l'Algérie, l'Irak, l'Iran et la Libye combinent une économie fortement dépendante aux énergies fossiles et un PIB insuffisant pour surmonter la perte de recettes tirées de la vente d'hydrocarbures \citep{muttitt_equity_2020}. Des pays comme le Nigeria, l'Angola et le Congo dépendent également fortement des fossiles, mais le revenu de base leur procurerait des transferts nets importants, grâce à leur faible empreinte carbone.% Libye aussi je pense mais les auteurs n'ont pas de données 
% Grâce à la clause dérogatoire leur permettant de conserver les recettes de la tarification carbone liées aux fossiles qu'ils mettent sur le marché mondial, ces pays récupéreraient une part des recettes qu'ils auraient totalement perdues en cas de décarbonation unilatérale du reste du monde. % TODO? put back?
...
% Et ce d'autant plus que la conversion de devises pour distribuer le revenu de base entraînera une appréciation du taux de change dans les pays à bas revenus, ce qui leur fera perdre en compétitivité et augmentera d'autant plus leurs importations. % TODO appréciation?
D'autre part, l'importance de secteurs en déclin (luxe et tourisme représentent à eux deux 6~\% du PIB français% https://www.letemps.ch/economie/luxe-redore-blason-leconomie-francaise
...
% pk pas demander d'acheter des permis? pck prix potentiellement déjà payé
% Pb: pour calculer le prix déjà payé, il faut des infos qui remontent la chaîne de valeur. 
% Je suis pas convaincu par ce prix contrefactuel: si les autres pays rejoignaient l'union, le prix interne aussi augmenterait. Une meilleure façon d'éviter les fuites de carbone serait d'ajuster le budget carbone de l'union en soustrayant ses importations nettes de carbone du quota mis aux enchères. Ça appliquerait le prix contrefactuel même en interne.
...
Ce livre est disponible gratuitement en version PDF à l'adresse~: \href{https://global-redistribution-advocates.org/fr/un-plan-mondial-pour-le-climat-et-contre-lextreme-pauvrete/}{bit.ly/gra\_pmc}. % TODO Put on SSRN and use SSRN on GRA site
...
\renewcommand{\url}[1]{\href{#1}{Lien}} % NCCcomment
{\small 
...
% PNON Alors pourquoi les pays n'agissent pas? Après mes multiples échanges, une des raisons est la suivante. Il y a trop peu de personnes qui travaillent sur ces sujets dans l'administration et dans les négociations climatiques. Ces gens-là font déjà avancer les choses, nouent des partenariats, avancent pas à pas pour changer les règles du système financier. Ils sont déjà occupés par leurs projets en cours, et il y a déjà de multiples recommandations de rapports officiels qui seraient utiles de mettre en œuvre. Tous ces efforts sont extrêmement ralentis, voire empêchés, par la défense de l'intérêt national de la part de nombreuses parties prenantes. En outre, un projet comme celui-ci leur paraît trop ambitieux car il rompt avec l'ordinaire incrémental de leurs actions. Pour résoudre ces obstacles, je pense que les pays du monde entier devraient rassembler un ou deux milliers d'universitaires travaillant sur ces sujets dans un réseau, et les faire travailler directement dans leurs ministères. Crucialement, les universitaires ne travailleraient pas nécessairement pour leur pays d'origine, favorisant ainsi les échanges d'information et l'émergence d'une vision commune (plutôt que nationale). En effet, les universitaires ont l'habitude de collaborer en transcendant les frontières et les institutions, contrairement aux ministères. De mon côté, ce sont trois passés à la prestigieuse université Tsinghua, à Pékin, qui m'ont permis de comprendre les contraintes et les ambitions de la Chine dans les négociations climatiques. Par ailleurs, les négociations internationales ne devraient pas se tenir à huis clos, mais être enregistrées et diffusées. Aussi, il devrait y avoir plus de discussions en parallèle, grâce à une délégation plus grande de la part des ministres, de sorte qu'on puisse avancer sur plusieurs fronts en même temps, plutôt que de focaliser les négociations sur un seul sujet (en ce moment, les MDBs).

% LASER TALK / FAQ / Répondre aux contre-arguments

% 1. Les Français d'abord: Il faut commencer par aider les Français avant de se soucier des gens loin de nous. Il y a déjà des gens qui n'arrivent pas à se nourrir en France, et on n'arrive déjà pas à les aider. Commençons par balayer devant notre porte.
% => L'un n'empêche pas l'autre. Dans ce qu'on propose, seul un tiers finance l'étranger. La solidarité entre pays est 100 fois faible que au sein d'un pays. 
%    C'est dans notre intérêt de le faire, pour pacifier le monde, éviter les conflits liés aux famines, qui ont des répercussions jusque chez nous, avec l'afflux de migrants.
%    L'Occident a une dette morale envers les pays du Sud à cause du CC. 

% 2. Pourquoi verser de l'argent à des dictateurs corrompus qui tuent leur population ?
% => Il faut un audit des comptes par une institution internationale où chaque pays est représentée, et un système de paiement au sein des administrations qui permet une traçabilité. Lutter contre la corruption nécessite des moyens.
%    Les fonds ne seront pas versés si ces conditions ne sont pas remplies.

% 3. Ça n'arrivera pas.
% => Lula propose un ISF mondial au G20. Des diplomates de nombreux pays sont d'accord. 
%    80% de la population est d'accord.

% 4. C'est déjà difficile de boucler nos fins de mois, on pense d'abord à nous.
% => Seuls les gens possédant plus de 5M€ paieront, soit 0.1% de la population.

% 5. Les riches partiront si on les taxe et ça réduira le PIB.
% => On ne taxera qu'à partir de 5M€, et au taux de seulement 2%. Les chiffres montrent que très peu de riches partent en pratique. On peut encore réduire les départs avec une exit tax.
%    Si c'est mondial, ils n'auront pas d'endroit où aller.
