...
\author{Adrien Fabre\footnote{The author is Adrien Fabre, CNRS researcher in economics at CIRED. E-mail: adrien.fabre@cnrs.fr.}
...
In representative surveys, \citet{fabre_international_2023} show that 54\% of Americans and 76\% of Europeans support the Global Climate Plan (GCP) as described above, while the support is as high as 96\% in China. 

In this policy brief, we make the case for a Global Climate Plan and foresee China and the EU as the coalition's leaders. We show that the GCP is grounded on solid ethics and economics (Section \ref{sec:principles}), would operate a global redistribution from rich to poor (Section \ref{sec:distribution}), can be implemented with current technology (Section \ref{sec:implementation}), and is genuinely supported by the population across the World (Section \ref{sec:support}). Finally, we expand on the above description and formulate a well-specified plan (Section \ref{sec:details}).
