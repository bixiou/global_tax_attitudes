...
%DIF DEL /tmp/claude-1000/-mnt-c-Users-fabre-Documents-www-global-tax-attitudes/5948215f-7446-4873-b298-86975652e23b/scratchpad/itmo_rules_short_old.tex   Mon Jul  6 20:42:45 2026
%DIF ADD paper/itmo_rules_short.tex                                                                                                                        Mon Jul  6 20:38:40 2026
...
I \DIFdelbegin \DIFdel{make the case for a coalition of the willing to jointly align its NDCs with the Paris target and commit not to exchange ITMOs with countries outside the coalition. I }\DIFdelend start by describing existing proposals by \cite{michaelowa_additionality_2019} and \cite{la_hoz_theuer_when_2019}, then propose different options for leveraging ITMOs to enhance climate ambition, with increasing levels of stringency (summarized in Table \ref{tab:proposals}). \DIFaddbegin \DIFadd{Among the rules I propose, some rely on national assessments of NDCs: principled buyers would commit not to buy ITMOs from a country with an unambitious NDC. Besides, the conditional NDC target of a Global South country would supersede its unconditional target in adequacy assessments once the country's conditions in terms of climate finance are met. Hence, when a country sets its unconditional target above its projected emissions and its conditional target below them, buyers would need to provide climate finance to that country before purchasing ITMOs from it, as the seller's NDC would otherwise not be ambitious enough. Finally, I detail the limitations of relying on national assessments and make the case for a coalition of the willing to jointly align its NDCs with the Paris target and commit not to exchange ITMOs with countries outside the coalition. }\DIFaddend % Table \ref{tab:proposals} summarizes these proposals. %Section \ref{sec:conclusion} concludes.
...
\cite{michaelowa_additionality_2019} propose verifying that ITMOs are additional, meaning they correspond to emission reductions relative to a counterfactual scenario without such ITMOs. The authors suggest a two-step procedure wherein a new ``Article 6 Supervisory Board'' would first check whether the seller's NDC is more ambitious than the \DIFaddbegin \DIFadd{(frequently updated) }\DIFaddend business-as-usual (BAU) trend. %DIF >  i.e. the projection of emissions under \textit{current policies} (as provided e.g. by the \href{https://climateactiontracker.org/}{Climate Action Tracker}). 
...
While \cite{la_hoz_theuer_when_2019} show that simple definitions of the BAU are ill-suited for regulating ITMOs, \cite{michaelowa_additionality_2019} propose delegating the determination of the BAU to independent experts so that NDC ambition can be accurately assessed.\footnote{Expert projections of BAU emissions already exist, see e.g. \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{den_elzen_impact_2023,den_elzen_infographics_2024}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{den_elzen_are_2019,den_elzen_impact_2023,den_elzen_infographics_2024,unep_emissions_2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend .} In both papers, the authors agree that an NDC can be assessed at the national level, by comparing it to the country's projected emissions, and that ITMOs should be allowed whenever the NDC is more ambitious than properly projected emissions. 
...
\subsection{A sine qua non condition: no ITMOs from failed NDCs\DIFdelbegin \DIFdel{.}\DIFdelend } % no ITMO from failed NDC
...
This principle should also apply to countries that have weakened their emission targets (instead of ratcheting up ambition), resulting in emissions higher than planned under a previous target \DIFdelbegin \DIFdel{.}\footnote{\DIFdel{Angola provides an example of a country that effectively decreased its ambition in an updated NDC, 
%DIF <  new
by revising upwards (by a factor two) its BAU emissions.}}
%DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{(cf. }\href{https://climateactiontracker.org/climate-target-update-tracker/list-non-updating-countries/}{\DIFadd{Climate Action Tracker} gives examples of such countries}\DIFadd{).
%DIF >  \footnote{Angola provides an example of a country that effectively decreased its ambition in an updated NDC, 
%DIF >  % new
%DIF >  by revising upwards (by a factor two) its BAU emissions.}
...
\subsection{Principled buyers: no ITMOs from unambitious NDCs\DIFdelbegin \DIFdel{.}\DIFdelend } %  no ITMO for NDC > cost-optimal/equal pc
...
principled buyers should refuse to buy ITMOs from a country whose NDC target is above the BAU\DIFaddbegin \footnote{\DIFadd{The BAU is understood as a projection of emissions which accounts for the most recent technological developments.}} 
  %DIF >  current policies; see \cite{den_elzen_are_2019,unep_emissions_2025} for methodologies assessing current policies scenarios} 
\DIFaddend or evidently incompatible with the Paris target, even when it is below the BAU.
% In absence of a common norm to disaggregate the Paris target at the country level, 
Such incompatibility could be safely assumed when a country's NDC target exceeds both its cost-optimal emissions required to limit global warming to 2\textdegree{}C and its equal per capita share of global emissions under that 2\textdegree{}C scenario. %DIF >  (thereby spanning a wide range of candidate burden-sharing rules, \citealp{van_den_berg_implications_2020}). 
Indeed, no credible burden-sharing rule could then justify the NDC's adequacy with the Paris temperature target %DIF > . This approach is in line with the literature assessing the ambition of NDCs against least-cost scenarios and equity outcomes
 \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{van_den_berg_implications_2020,robiou_du_pont_equitable_2017,hohne_regional_2014}}\hskip0pt%DIFAUXCMD
}\DIFaddend . % Countries affected by this additional requirement are large emitters setting unambitious NDC targets, such as China. % check if this is the case of Thailand => NICE global C&S 2°C, 1.5°C. 284/422=67% 2024-THA emissions excl. LULUCF are fossil CO2 according to EDGAR; according to Thailand: 163/278=59% of CO2 over GHG (271 non-LULUCF CO2, 163 CO2 incl. LULUCF, 386 GHG excl. LULUCF, 278 incl. LULUCF) https://unfccc.int/sites/default/files/resource/THAILAND%E2%80%99S%20BTR1.pdf; 
...
Thailand appears to meet all aforementioned criteria, suggesting that Switzerland could be considered a principled buyer. However, until the sum of all countries' NDCs aligns with the Paris target, the price of ITMOs might be too low. In other words, the above requirements are necessary but not sufficient conditions to close the ambition gap\DIFaddbegin \DIFadd{, understood here as the gap between the Paris target and the sum of NDCs \mbox{%DIFAUXCMD
\citep{unep_emissions_2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend . % hotair


\subsection{Adequate climate finance unlocking conditional NDCs\DIFdelbegin \DIFdel{.}\DIFdelend } % {Sellers' conditions
While these requirements would reduce the amount of hot air, they would exclude from the trade of ITMOs low-emitting countries that set their NDC target above the BAU,\footnote{For example, \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{van_den_berg_implications_2020} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{van_den_berg_implications_2020} and \cite{unep_emissions_2025} }\hskip0pt%DIFAUXCMD
}\DIFaddend conclude that India has set its 2030 NDC above the BAU \DIFaddbegin \DIFadd{(note that the BAU trajectory was computed by the authors; India's NDC does not specify a BAU)}\DIFaddend . %DIF <  (note that the BAU trajectory was computed by the authors; India's NDC does not specify a BAU). % revision: put back
%DIF >  revision: put back
} 
...
\subsection{Issues with national assessments of NDCs\DIFdelbegin \DIFdel{.}\DIFdelend }
...
Therefore, the ratcheting up of ambition should arguably be borne by industrialized countries \DIFaddbegin \DIFadd{(including China), which should decarbonize faster and provide climate finance abroad}\DIFaddend . For these reasons, assessing an NDC against the national (or project-based) BAU is neither a fair nor effective way to close the ambition gap. % prevent warm air. 
...
The key problem with ITMOs is that the \textit{global} emissions implied by current NDCs (or current trends) do not align with the Paris temperature target \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{unep_emissions_2025,rogelj_credibility_2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
\subsection{Limitations of restrictions to the use of ITMOs\DIFdelbegin \DIFdel{.}\DIFdelend }
...
\section{\DIFdelbegin \DIFdel{The case for a joint definition }\DIFdelend \DIFaddbegin \DIFadd{A coalition }\DIFaddend of \DIFaddbegin \DIFadd{the willing to jointly define }\DIFaddend NDCs\DIFaddbegin \label{sec:itmo}\DIFaddend }
\DIFaddbegin 

\subsection{\DIFadd{The case for a joint definition of NDCs}}\DIFaddend \label{subsec:itmo_proposal}
...
This coalition could also allocate its aggregate NDC between countries in a way deemed acceptable and fair, potentially allowing certain countries to get a target higher than their \DIFdelbegin \DIFdel{BAU }\DIFdelend \DIFaddbegin \DIFadd{current policies }\DIFaddend emissions.
...
\DIFdelbegin \section{\DIFdel{A coalition for the use of ITMOs}%DIFDELCMD < \label{subsec:itmo}%%%
}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{A coalition for the use of ITMOs}\label{subsec:itmo}}
\DIFaddend 
...
Once a large coalition of countries agree on the broad vision, this coalition could be taken as given. The coalition's emissions targets would be set below its projected emissions, with a gradually increasing wedge between \DIFdelbegin \DIFdel{BAU }\DIFdelend \DIFaddbegin \DIFadd{current policies }\DIFaddend emissions and the target. Specifically, the 2026 target would correspond to the coalition's emissions, %the 2027 target would be slightly below the BAU, the 2028 more so, etc., realistically increasing the additional decarbonization effort overtime, TODO?
while the 2027 target would be slightly below the \DIFdelbegin \DIFdel{BAU}\DIFdelend \DIFaddbegin \DIFadd{current policies projection}\DIFaddend , increasing the divergence in 2028 and beyond. This would realistically scale up the additional decarbonization effort over time 
...
The resulting emission trajectory of the coalition should not exceed its equal per capita share of a global carbon budget achieving 2\textdegree{}C with a \DIFdelbegin \DIFdel{75}\DIFdelend \DIFaddbegin \DIFadd{67}\DIFaddend \% probability,\footnote{That is equivalent to \DIFdelbegin \DIFdel{1.8}\DIFdelend \DIFaddbegin \DIFadd{1.9}\DIFaddend \textdegree{}C with a 50\% probability \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{forster_indicators_2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend .} and would ideally be lower. % harmonize carbon budget with above => 1.8°-50%
...
% scientific council
A scientific council would assist the coalition by determining \DIFdelbegin \DIFdel{BAU }\DIFdelend \DIFaddbegin \DIFadd{current policies }\DIFaddend emissions, modelling climate and economic effects, and proposing target allocations. Each participating country would designate a team of scientists (which could overlap), with voting rights proportional to the country's population in the event of disagreement.
...
\DIFdelbegin \section{\DIFdel{Feasibility of the previous proposal}%DIFDELCMD < \label{subsec:feasibility}%%%
}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsection{\DIFadd{Feasibility of the previous proposal}\label{subsec:feasibility}}
\DIFaddend 
...
A tighter regulation of ITMOs combined with an ambitiously expanded JETP program \citep{steckel_climate_2017,bolton_why_2025} offers \DIFdel{the most}\DIFdelend \DIFadd{a }\DIFaddend viable pathway for effective international decarbonization. In both cases, a coalition of the willing seems the best option to reap the gains from the cooperation of ambitious countries. 
