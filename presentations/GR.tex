...
\begin{center}
\begin{LARGE}
\rose{Global Redistribution\\}
\blue{Placing the Issue on the Agenda}
\end{LARGE}
...
\begin{frame}{Huge inequality between countries}
    \begin{figure}
        \centering 
        \caption{GDP per capita relative to the world average, adjusted for cost of living (2022, World Bank)}
        \includegraphics[height=.8\textheight]{../figures/policies/GDP_pc_PPP_few.pdf} 
    \end{figure}  
\end{frame}

\subsection{Public support}

\begin{frame}{Support for plausible gobal redistribution\label{solidarity_support_absolute}}
\centering Share of (somewhat or strong) \textit{support} among non-\textit{indifferent} answers. \\
\makebox[\textwidth][c]{ \includegraphics[width=.89\textwidth]{../../robustness_global_redistr//figures/country_comparison/solidarity_support_share}}
\end{frame}

\begin{frame}{Support for radical gobal redistribution\label{solidarity_support_absolute}}
\centering Share of (somewhat or strong) \textit{support} among non-\textit{indifferent} answers (except for binary questions on \textit{convergence} and \textit{sustainable  future}). \\
\makebox[\textwidth][c]{ \includegraphics[width=\textwidth]{../../robustness_global_redistr//figures/country_comparison/radical_redistr_some_share}}
\end{frame}
...
%     \ip \blue{ITMOs} may weaken domestic action if joint NDCs from the countries exchanging ITMOs if they aren't Paris-aligned. To prevent that:
...
\begin{frame}{A Global Climate Scheme (GCS)\label{ffu} }
    \only<7>{
    ...
    \begin{enumerate}[<+->]
        ...
        \bbvsp \ip A carbon market, decreasing emissions permits overtime, auctioned globally to fossil fuel companies. %$\Rightarrow$ Define a carbon budget and allowances trajectory by country.
        ...
        \bbvsp 
        \ip A price of \$90/tCO$_\text{2}$ could finance a cash transfer of $\approx$\rose{\$20 per month} (in nominal) \rose{for each human} and lift out of extreme poverty the 700 million people with less than \$2.15 a day (in PPP).
        % \ip \rose{Benchmark} carbon budget for a country given by \rose{equal rights per capita} (using initial population).
        \ip \blue{Departures from the} \rose{benchmark equal rights per capita} to account for peculiar needs and contexts:
        \ip Adjusted carbon budget for countries with \rose{higher} \green{needs} or \blue{ambition}: 
        \\ \green{China} and \blue{the EU} are \rose{granted allowances corresponding to their decarbonization pathways}. 
...
\makebox[\textwidth][c]{ \includegraphics[width=\textwidth]{../../robustness_global_redistr/figures/country_comparison/variables_ncs_gcs_ics_control_by_country}} 
\end{frame}
...
\ip \blue{\href{https://github.com/bixiou/robustness_global_redistr/raw/main/papers/support_global_redistr.pdf}{Acceptance for International Redistribution in High-Income Countries}}
\ee 
...
\makebox[\textwidth][c]{ \includegraphics[width=\textwidth]{../../robustness_global_redistr/figures/country_comparison/transfer_how_positive}}
\end{frame}
