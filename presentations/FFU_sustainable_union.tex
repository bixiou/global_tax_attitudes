...
\end{LARGE}
% TODO! add GDP per country blue bars
...
\begin{frame}{Limitations of the current int'l climate policy regime}
    \bbsp \ip \blue{CBDR} is vague, doesn't specify the burden sharing rules.
    \ip \blue{NDCs} are not Paris-aligned and don't use a common indicator (e.g. a carbon budget).
    \ip \blue{ITMOs} may weaken domestic action if joint NDCs from the countries exchanging ITMOs aren't Paris-aligned. To prevent that:
    ...
    \ip \blue{Climate finance} is necessary but not sufficient as it doesn't guarantee decarbonization.
    \ip \blue{JETPs} can provide climate finance but they don't address poverty and do nothing to decarbonize big emitters like China or the EU.
    \ip $\Rightarrow$ We need a regime that: is Paris-aligned \rose{temperature}-wise; meets the \rose{SDGs}; is \rose{efficient}; and \rose{acceptable} (i.e.  makes the richest pay; avoids losses in MICs; and is win-win as LICs enjoy transfers and HICs efficiency gains from trade.).
    \ee
\end{frame}
...
\begin{frame}{Global policies are strongly supported by the public.\label{global_policies}}
	...
	\begin{figure}[h!]
        ...
        \vspace{-.2cm}
		\includegraphics[height=.9\textheight]{../figures/OECD/Heatplot_global_tax_attitudes_share_wo_TD.png} % Heatplot_global_tax_attitudes_share_wo_TD.png Heatplot_global_tax_attitudes_share.pdf
		\end{figure}
\end{frame}
...
\begin{frame}{A Global Climate Scheme\label{ffu} }
    \only<9>{
        \centering{ \large \textbf{The proposed allocation of emissions allowances}} \vspace{-.2cm}
        \begin{table}[h]
            \caption{Carbon budget over 2030--2080 for a 1.8\textdegree{}C trajectory (in GtCO$_\text{2}$)} \vspace{-.5cm}
            \makebox[\textwidth][c]{
            \begin{tabular}[t]{ccccccccc}
            \toprule
             & Africa & China & Latin America & India & Europe & Other Asia & \textit{Union} & \textit{World} \\
            \midrule
            \textbf{Equal p.c.} & 144 & \blue{131} & 64 & 140 &  \blue{51} & 134 &  \textit{691} & \textit{770} \\
            \textbf{Proposal} & 146 & \blue{147} & 64 & 140 & \blue{24} & 139 & \textit{689} & \textit{770} \\        
            \bottomrule\\[-0.81em]
            \end{tabular}}
          \end{table}
        \begin{figure} \vspace{-.5cm}
            ...
            \includegraphics[height=.6\textheight]{../figures/policies/ffu_rights_pc_price_revenue.pdf} % fossil_free_union_emission_trajectories.png
        \end{figure}
    }
    \begin{enumerate}[<+->]
        \item \blue{A cap on emissions to meet the Paris target.}
        \bbvsp \ip A carbon market, with emissions permits auctioned to fossil fuel companies at the international level. %$\Rightarrow$ Define a carbon budget and allowances trajectory by country.
        ...
        \ip Rule to guarantee that emissions permits decrease overtime and do not exceed emissions at start year.%A steadily increasing \rose{carbon price floor starting at \$10/tCO$_\text{2}$} to ensure decarbonization incentives.
        \ee
        ...
        \item \blue{Revenues allocated on a per capita basis (with some adjustments). \hyperlink{distributive}{\beamergotobutton{See distributive effects}}}
        \bbvsp \ip \rose{Benchmark} carbon budget for a country given by \rose{equal rights per capita} (using initial population).
        \ip \rose{Departures from the benchmark} to account for peculiar needs and contexts:
        \ip Adjusted carbon budget for countries with \rose{higher} \green{needs} or \blue{ambition}: 
        \\ \green{China} and \blue{the EU} are \rose{granted allowances corresponding to their decarbonization pathways}. \pause \pause
        ...
        \bbvsp \ip The first signatories could be Brazil, China, and the African Union.  \hyperlink{scenarios}{\beamergotobutton{See union scenarios}} %Entry into force when signatories cover 60\% of global emissions.
...
% \begin{frame}{The proposed allocation of emissions allowances \hyperlink{distributive}{\beamergotobutton{Distributive effects}}}
%     \begin{table}[h]
%         \caption{Carbon budget over 2030--2080 for a 1.8\textdegree{}C trajectory (in GtCO$_\text{2}$)} \vspace{-.4cm}
%         \makebox[\textwidth][c]{
%         \begin{tabular}[t]{ccccccccc}
%         \toprule
%          & Africa & China & Latin America & India & Europe & Other Asia & \textit{Union} & \textit{World} \\
%         \midrule
%         \textbf{Equal p.c.} & 144 & \blue{134} & 62 & 135 &  \blue{49} & 128 &  \textit{653} & \textit{754} \\
%         \textbf{Proposal} & 144 & \blue{154} & 62 & 135 & \blue{23} & 128 & \textit{647} & \textit{748} \\        
%         \bottomrule\\[-0.81em]
%         \end{tabular}}
%       \end{table}
...
\makebox[\textwidth][c]{ \includegraphics[width=\textwidth]{../../robustness_global_redistr/figures/country_comparison/variables_ncs_gcs_ics_by_country}} 
\end{frame}

\begin{frame}{Alternative proposals}\label{alternatives}
    \bbsp \ip \rose{Fossil-fuel non-proliferation treaty} by Calverley \& Anderson (2022); climate movement. \\
    Supply-side policy: phase out fossil fuel production, starting with richest countries. \\
    $\Rightarrow$ \blue{Relies on fuel producers' cooperation}; \blue{increases oil rents} and hurt consumers; lacks efficiency.
    \ip \rose{Differentiated carbon prices} by Parry, Black \& Roaf (IMF, 2021), Wolfram et al. (2025). \\
    Higher prices in HICs, no transfer, carbon tariff at the club's border. \\
    $\Rightarrow$ Limited ambition; few transfers; \blue{no gains from trade}. \hyperlink{correspondence}{\beamergotobutton{Correspondence rights - differentiated prices}}
    \ip \rose{Pricing incentive reward} by Stoft (2009). \\
    Transfers proportional to price deviation from target and emissions below world average. \\
    $\Rightarrow$ No guarantee to achieve temperature target; \blue{HICs may be net recipient}. 
    \ip \rose{Refunding club} by Gersbach et al. (2021), Finus (2024). \\
    Initial (potentially negative) payments and yearly refunds proportional to abatement. \\
    $\Rightarrow$ \blue{Initial payments difficult} to finance.
    \ee
\end{frame}

\begin{frame}{Support for global justice policies\label{support}\hyperlink{support_more}{\beamergotobutton{More results}}}
	\vspace{-.3cm}
	\begin{figure}[h!]
		\centering		
        ...
        \vspace{-.2cm}
		\includegraphics[width=\textwidth]{../figures/country_comparison/few_main_share.pdf} % burden_share_all_share_countries
		\end{figure} 
\end{frame}
...
\begin{frame}{Principles and proposals}
    \bbsp \ip Accept \rose{non-universal agreements}.
    ...
    %\ip \rose{Carbon price floor} at \$10/tCO$_\text{2}$ evolving into an emissions trading system.
...
\begin{frame}{A pragmatic treaty}
    ...
    \bbsp \ip Revenues from \rose{new taxes}, e.g. \blue{2\% of GDP}.
    \ip \rose{International transfers} for sustainable development, e.g. \blue{1\% of GDP}.
    \ip A \rose{global carbon budget}, e.g. \blue{1,000 GtCO$_\text{2}$}.
    \ee
    % \ip \rose{Carbon price floor} at \$10/tCO$_\text{2}$.
    % \ip Return CBAM revenues to exporting country.
    \ip \blue{Flexibility} and \rose{conditional cooperation}:
...
\begin{frame}{What next?}
    \bbsp
    \ip \blue{Understand} what type of deal each country would accept.
    \ip \blue{Design} a precise plan acceptable by most countries.
    \ip Foster an international coalition to \blue{campaign} on this plan.
    \ee
\end{frame}
...
~\\ \centering{For further information:}
\bbvs
\ip \blue{\href{https://github.com/bixiou/global_tax_attitudes/raw/main/paper/global_climate_policies.pdf}{From Global Policies to Phase Out Fossil Fuels To a Sustainable Union}}
% \ip \blue{\href{https://github.com/bixiou/global_tax_attitudes/raw/main/paper/Fossil\%20Fuel\%20Phase\%20Out\%20Treaty.pdf}{Fossil-Free Union - 2-pager}}
% \ip \blue{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5141096}{A shared vision towards global climate justice}} \\ (longer version, by a worldwide group of scholars and diplomats)
% \ip \blue{\href{https://github.com/bixiou/global_tax_attitudes/raw/main/paper/The\%20Sustainable\%20Union\%20-\%20draft\%20treaty.pdf}{The Sustainable Union -- Draft Treaty}}
\ip \blue{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4448523}{Majority Support for Global Climate and Redistributive Policies}}\\ 
(\textit{Nature Human Behaviour}; Fabre, Douenne \& Mattauch; 2025)
\ee 
%\textbf{Thank you for attention!}
% 
% ~\\ \quad \\ \quad \\ %\blue{Livre: \href{https://bit.ly/plan_mondial_climat}{bit.ly/plan\_mondial\_climat}} 
\centering{Book: \blue{\href{https://bit.ly/bookGCP}{bit.ly/bookGCP}} \\  (free, preface by Gabriel Zucman)}
\begin{figure}
    \includegraphics[height=.18\textheight]{../book/cover_en_title.PNG} 
\end{figure}    
\centering @adrien\_fabre 
\end{frame}

\appendix

\section{Correspondence between\\ rights 
and differentiated prices}

\begin{frame}{Correspondence between rights in uniform price ($U$) and differentiated prices ($A$) \hyperlink{alternatives}{\beamergotobutton{Go~back}}\label{correspondence}}
\bbsp \ip \blue{Static case}. $p$: carbon price; \rose{$r_i$}: $i$'s emission \rose{rights}; $\green{\pi_i} = p_i/p$: \green{differentiated price factor} in $i$; $e_i=e_i(p)$: emissions in country $i$; $\varepsilon = -\frac{de_i/e_i }{dp/p}$: price elasticity of emissions:

\centering $\rose{r_i} - e_i \sim (1 - \green{\pi_i}) e_i \varepsilon$

\ip Extra emission rights (transfers) equivalent to lower carbon price, absent efficiency gains. 

\ip Efficiency gains $\Rightarrow$ \textit{U}niform price with transfers $\succ$ \textit{A}utarchic differentiated prices.

\ip \blue{Dynamic Hotelling, homothetic case}: Assume $p_{it}=\pi_i p_t$, $r_{it}=r_i r_t$; \\ fixed carbon budget, perfect foresight $\Rightarrow$ discounted price is constant: $\beta_t p_t = p_0$. \\
Capital letters denote intertemporal sums. 

\centering    $\rose{R_i} - E_i \sim (1 - \green{\pi_i}) \sum_t e_{it}  \varepsilon_t$
\ip \blue{General dynamic case}: \rose{no immediate relation} between $i$'s carbon budget $R_i$ and its welfare: the \rose{price trajectory matters} $\Rightarrow$ equivalence can be computed numerically.
\ee
\end{frame}

\section{Support for global justice policies}

\begin{frame}{International surveys with a focus on the West\label{support_more} \hyperlink{support}{\beamergotobutton{Go back}}}
    \bbvs
    \ip \green{\textbf{Global survey}}  (02/2021--02/2022) by \rose{Dechezleprêtre et al. (2022)}, \textit{American Economic Review}
...
\begin{frame}{The \textit{Global Climate Scheme} (GCS)\label{GCS_def} \hyperlink{support}{\beamergotobutton{Go back}}}
    \bbvs \ip Our main policy of interest is the GCS, a \rose{global emissions trading system funding a global basic income}:
    \bbvs \ip \small{ At the Paris agreement in 2015, all countries have agreed to contain global warming ``well below +2 $\mathrm{{}^\circ}$C''. To limit global warming to this level,~\textbf{there is a maximum amount of greenhouse gases we can emit globally}.\\ \quad \\
    To meet the climate target, a limited number of permits to emit greenhouse gases can be created globally. Polluting firms would be required to buy permits to cover their emissions. Such a policy would~\textbf{make fossil fuel companies pay}~for their emissions and progressively raise the price of fossil fuels.~\textbf{Higher prices would encourage people and companies to use less fossil fuels, reducing greenhouse gas emissions.}\\ \quad \\
    In accordance with the principle that each human has an equal right to pollute, the revenues generated by the sale of permits could finance a global basic income.~\textbf{Each adult in the world would receive \$30/month}, thereby lifting out of extreme poverty the 700 million people who earn less than \$2/day.\\ 
...
\begin{frame}{Support for the Global Climate Plan\label{gcs_support} \hyperlink{support}{\beamergotobutton{Go back}}}
\bbvs 
...
%\ip To offset the losses of the median emitter, we design a \rose{National Redistribution Scheme} (NR) funded by increased income taxes on the top 5\% (US) / 1\% (Eu).
...
\begin{frame}{List experiment\label{} \hyperlink{support}{\beamergotobutton{Go back}}}
    \vspace*{-.1cm}
...
\begin{frame}{Conjoint analyses: influence on electoral prospects\label{conjoint_c} \hyperlink{support}{\beamergotobutton{Go back}}} 
    % \begin{figure}
    %     \centering 
    ...
    \begin{table}[h]\label{tab:conjoint_c_wo_none}\vspace*{-.2cm}
        ...
        \makebox[\textwidth][c]{\small \input{../tables/country_comparison/conjoint_c_wo_none.tex}}
...
\begin{frame}{Conjoint analyses: influence on preferred platform (UK)\label{conjoint_r_uk} \hyperlink{support}{\beamergotobutton{Go back}}} 
	%\vspace{-.2cm}
...
\end{itemize}
\makebox[\textwidth][c]{
\includegraphics[width=\textwidth]{../../robustness_global_redistr/figures/country_comparison/sustainable_future_positive.pdf}} 
% \bbvs
% \ip 
% \ee
\end{frame}
...
\makebox[\textwidth][c]{ \includegraphics[width=\textwidth]{../../robustness_global_redistr/figures/country_comparison/global_movement_all_positive.pdf}}
...
\makebox[\textwidth][c]{ \includegraphics[height=.9\textheight]{../../robustness_global_redistr/figures/country_comparison/vote_intl_coalition.pdf}}
% \bbvs
% \ip 
% \ee
\end{frame}
...
\makebox[\textwidth][c]{ \includegraphics[height=.9\textheight]{../../robustness_global_redistr/figures/country_comparison/group_defended}}
% \bbvs
% \ip 
% \ee
\end{frame}
...
\begin{frame}{Support for other global policies\label{other_policies}  \hyperlink{support}{\beamergotobutton{Go back}}}
	\vspace{-.3cm}
    \begin{figure}
        \centering 
...
\begin{frame}{Support for a global wealth tax\label{wealth_tax}  \hyperlink{support}{\beamergotobutton{Go back}}}
...
\begin{frame}{Support for increased foreign aid \hyperlink{support}{\beamergotobutton{Go back}}\label{foreign_aid_perceptions}}\vspace{-.2cm} 
    \begin{figure} 
        \centering 
...
\begin{frame}{Preferences over public spending%How to fund increased foreign aid
	\hyperlink{support}{\beamergotobutton{Go back}}}
	\begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{figure}
				\vspace{-.2cm}
...
%         \ee
%         \item \blue{A climate union led by the Global South.}
%         \bbvsp \ip The first signatories could be Brazil, China, the African Union, and Mexico. %Entry into force when signatories cover 60\% of global emissions.
...
\begin{frame}{The distributive effects\label{distributive} \hyperlink{ffu}{\beamergotobutton{Go back}}}
    \begin{figure}
        \centering 
...
\begin{frame}{Scenarios with non-universal participation\label{scenarios} \hyperlink{ffu}{\beamergotobutton{Go back}}}
\input{../tables/scenarios_table.tex}
\end{frame}

\begin{frame}{Scenarios with non-universal participation\label{scenarios_maps} \hyperlink{ffu}{\beamergotobutton{Go back}}}
    \begin{figure}
        \centering 
