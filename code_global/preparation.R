...
convert <- function(e, country, wave = NULL, weighting = T, zscores = T, zscores_dummies = FALSE, efa = FALSE, combine_age_50 = T, only_finished = T, define_var_lists = T) {
  ...
  if ("vote_us_voters" %in% names(e)) {
    ...
    # # A better definition (used in robustness_global_redistr) would have been the following. Fortunately, it doesn't change anything about the result (same % of gcs_support among swing_state, almost same in democratic_state)
    ...
    # e$democratic_state_new <- floor(n(e$zipcode)/1000) %[]% c(80, 81) | floor(n(e$zipcode)/100) %[]% c(900, 994) | floor(n(e$zipcode)/1000) %[]% c(5, 6) | floor(n(e$zipcode)/1000) %[]% c(60, 62) | floor(n(e$zipcode)/100) %[]% c(10, 29) | floor(n(e$zipcode)/1000) %[]% c(10, 14) | floor(n(e$zipcode)/100) == 5 | floor(n(e$zipcode)/100) %[]% c(197, 205) 
  ...
  if (only_finished) {
    ...
    if (country == "US2" & wave != "pilot") { # poverty_field
    ...
    if ("comment_field" %in% names(e) & wave != "pilot") {
...
##### Run #####
# Extended sample
us1a <- prepare(country = "US1", weighting = FALSE, exclude_speeder = F, only_finished = F, exclude_screened = F)
...
# Pilots extended
eupa <- prepare(country = "EU", wave = "pilot", weighting = FALSE, exclude_speeder = F, only_finished = F, exclude_screened = F)
us1pa <- prepare(country = "US1", wave = "pilot", weighting = FALSE, exclude_speeder = F, only_finished = F, exclude_screened = F)
...
# Export codebooks
for (k in c("eu", "us1", "us2", "all")) export_codebook(eval(str2expression(k)), paste0("../data/codebook_", k, ".csv"), stata = FALSE) # "eup", "us1p", "us2p", "ep"


##### Create covariates' lists #####
