# Running preparation.R lasts ~1 min
...
create_p <- function(ppp_year = 2017, pop_iso = pop_iso3, rescale = FALSE) { 
  data <- read.csv(paste0("../data/poverty/Povcalnet ", ppp_year, ".csv")) # https://datacatalogfiles.worldbank.org/ddh-published/0063646/DR0090251/world_100bin.csv?versionId=2023-05-31T15:19:01.1473846Z on https://datacatalog.worldbank.org/search/dataset/0063646
  ...
  gdp_pc <- read_excel("../data/poverty/gdp_pc_ppp.xls") # Fetched in 2023 NY.GDP.PCAP.PP.KD
  ...
  # Add country name
  iso3 <- read.csv("../data/poverty/country_iso3.csv")
