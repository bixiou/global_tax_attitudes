...
HIC <- c(pg$code[pg$gdp_pc_2019 > 13205], "TWN") # approximately right (63 countries instead of 81 but differences are due to small islands)  
...
emissions_tot <- emissions_pc <- c() # Used to define "rights"
...
# emissions_cc: rights-based, starts in 2030 at BAU and converges in 2050 at equal p.c. rights.
# emissions_target: equal_pc=rights for HIC (except WEU), BAU for LMIC (except CHI), emissions_cf for WEU, He et al. (2022) trajectory for CHI => He et al. gives 154 territorial, we use instead 147 from emissions_ndc to approach consumption-based
...
# emissions_ndc: emissions in the NDC-LTT scenario of van de Ven et al. (23)
...
for (y in years_v) v[[paste0("emissions_ndc30_", y)]] <- 1e6*v[[paste0("emissions_ndc30_", y)]]
# Create union
...
for (y in years_v) v[[paste0("emissions_ndc30_pc_", y)]] <- v[[paste0("emissions_ndc30_", y)]]/v[[paste0("pop_", y)]]
# Add GCP
df$region_tiam <- c("ODA", "AFR", "EEU", "MEA", "CSA", "FSU", "AUS", "WEU", "FSU", "AFR", "WEU", "AFR", "AFR", "ODA", "EEU", "MEA", "CSA", "EEU", "FSU", "CSA", "CSA", "CSA", "CSA", "MEA", "ODA", "AFR", "AFR", "CAN", "WEU", "CSA", 
                    ...
                    "ODA", "AUS", "MEA", "ODA", "CSA", "CSA", "ODA", "ODA", "EEU", "ODA", # If PRK is at the end, remove that last "ODA" and put it at the end
                    "WEU", "CSA", "MEA", "EEU", "FSU", "AFR", "MEA", "AFR", "AFR", "ODA", "AUS", "AFR", "CSA", "AFR", "EEU", "AFR", "CSA", "EEU", "EEU", "WEU", "AFR", 
                    "MEA", "AFR", "AFR", "ODA", "FSU", "FSU", "ODA", "CSA", "AFR", "MEA", "CHI", "AFR", "AFR", "FSU", "CSA", "USA", "FSU", "CSA", "ODA", "AUS", "AUS", "MEA", "AFR", "AFR", "AFR")
...
v$emissions_target_2045[v$region == "CHI"] <- 3.8*1e9
v$emissions_target_2050[v$region == "CHI"] <- 2.0*1e9
v$emissions_target_2055[v$region == "CHI"] <- 0.7*1e9
...
for (y in years_v) v[[paste0("emissions_bau_pc_", y)]] <- v[[paste0("emissions_bau_", y)]]/v[[paste0("pop_", y)]]
for (y in seq(2025, 2055, 10)) v[[paste0("pop_", y)]] <- (v[[paste0("pop_", y-5)]] + v[[paste0("pop_", y+5)]])/2
for (y in seq(2020, 2055, 5)) v[[paste0("emissions_target_pc_", y)]][v$region == "CHI"] <- (v[[paste0("emissions_target_", y)]]/v[[paste0("pop_", y)]])[v$region == "CHI"]
...
types <- c("emissions_cc", "emissions_cf", "emissions_bau", "rights", "rights_proposed", "emissions_formula", "emissions_target", "emissions_ndc", "emissions_ndc30", "cumulative_rights", "emissions_gcp", "pop")
...
lines( years_v[2:7], colSums(v[v$region %in% setdiff(regions_union, c("CHI", "WEU")), paste0("rights_proposed_",  years_v[2:7])])/colSums(v[v$region %in% setdiff(regions_union, c("CHI", "WEU")), paste0("pop_",  years_v[2:7])]), type = 'l', col = 'black', lwd = 4, lty = 1, xlab = "", ylab = "")
...
lines( years_v[2:7], v[v$region == "union", paste0("emissions_bau_pc_",  years_v[2:7])], type = 'l', col = 'black', lwd = 3, lty = 2, xlab = "", ylab = "")
lines( c(seq(2030, 2060, 5), 2070, 2080), v[v$region == "CHI", paste0("rights_proposed_pc_",  c(seq(2030, 2060, 5), 2070, 2080))], type = 'l', col = 'red', lwd = 2, lty = 1, xlab = "", ylab = "")
# lines( years_v[2:7], v[v$region == "CHI", paste0("emissions_bau_pc_",  years_v[2:7])], type = 'l', col = 'red', lwd = 2, lty = 2, xlab = "", ylab = "")
# lines( seq(2020, 2050, 5), v[v$region == "CHI", paste0("emissions_target_pc_",  seq(2020, 2050, 5))], type = 'l', col = 'red', lwd = 2, lty = 3, xlab = "", ylab = "")
lines( years_v[2:7], v[v$region == "IND", paste0("rights_proposed_pc_",  years_v[2:7])], type = 'l', col = 'orange', lwd = 2, lty = 1, xlab = "", ylab = "")
lines( years_v[2:7], v[v$region == "IND", paste0("emissions_bau_pc_",  years_v[2:7])], type = 'l', col = 'orange', lwd = 2, lty = 2, xlab = "", ylab = "")
# lines( years_v[2:7], v[v$region == "CSA", paste0("rights_proposed_pc_",  years_v[2:7])], type = 'l', col = 'blue', lwd = 2, lty = 1)
# lines( years_v[2:7], v[v$region == "CSA", paste0("emissions_bau_pc_",  years_v[2:7])], type = 'l', col = 'blue', lwd = 2, lty = 2)
lines( years_v[2:7], v[v$region == "AFR", paste0("rights_proposed_pc_",  years_v[2:7])], type = 'l', col = 'cyan', lwd = 2, lty = 1, xlab = "", ylab = "")
lines( years_v[2:7], v[v$region == "AFR", paste0("emissions_bau_pc_",  years_v[2:7])], type = 'l', col = 'cyan', lwd = 2, lty = 2, xlab = "", ylab = "")
...
sum(df[df$code %in% c(central, "KOR"), paste0("emissions_", 2030:2080)])/1e9 # 525 Gt: club emissions implied by price_1.8
sum(bau[bau$code %in% central, paste0("emissions_", 2030:2080)])/1e9 # 1394 Gt: club BAU emissions (921 Gt without CHN)
(club_emissions_share <- sum(df$emissions_2025[setdiff(df$code %in% c(union_high, "TUR", "CHL", "URY", "TUN", "UGA", "SDN", "MYS", "UKR", "TKM"), "CAN")])/sum(df$emissions_2025)) # 74%
sum(df$emissions_2025[df$code %in% c("USA", "AUS", "RUS", "SAU", "KAZ", "ISR", "ARE", "QAT", "SGP", "KWT", "OMN", "BHR", "HKG")])/sum(df$emissions_2025)
sum(df$emissions_2025[df$code %in% c("USA", "AUS", "RUS", "SAU")])/sum(df$emissions_2025)
sum(df$emissions_2025[df$code %in% c("AUS")])/sum(df$emissions_2025)
...
70+142+45+8+109+10+101+13+11 # 509 Gt emissions_cf (BAU converging to min(BAU; equal pc) in 2048) except for WEU: NDC (which we can call a BAU)
124+156+46+9+115+11+7+104+4+13 # Min non-losing rights, 589 Gt (central+SKO) emissions_cc (BAU converging to equal pc in 2050) except for WEU: NDC (which we can call a BAU)
...
sum(df$emissions_2025[df$country %in% c("Algeria", "Kazakhstan", "Iraq", "Iran", "Libya", "Mongolia", "South Africa", "Turkmenistan")])/sum(df$emissions_2025)
653*sum(df$pop_2030[df$country %in% c("Algeria", "Kazakhstan", "Iraq", "Iran", "Libya", "Mongolia", "South Africa", "Turkmenistan")])/sum(df$pop_2030)


##### Country downscaling of emissions rights #####
# Equal pc (using 2030 pop) within world region, keeping the region trajectory => no, e.g. some in Africa deserve equal pc. For EU, an alternative would be to use Effort-Sharing Regulation allocation but TODO later.
# a. Group countries by similarity (e.g. put ZAF, IRN, MNG with CHN), manually adjust, downscale equal pc.
# b. Find formula (cf. intertemporal_allocation.lyx)
# For Algeria, Kazakhstan, Iran, Iraq, Libya, Mongolia, South Africa, and Turkmenistan, give them their rights rescaled by their emissions/average in 2025, taken out of the 12 Gt. 
# For 2030-40: for non-Union ones, no pb as rights_proposed increase not more than BAU; for AFR/ODA ones, taken from other of the regions and compensated in 2050-60 from 12 Gt.
# Compute Union emissions accounting for new members. Ask Erwan to infer carbon price and emissions by country for each period.
# From emissions rights and emissions by period by country, maps of int'l transfers (in % of GDP) in 2030, 40, ..., 70, as well as NPV.
...
for (y in 2030:2080) df[[paste0("rights_", y)]] <- (df$pop_2030/sum(df$pop_2030)) * # /!\ rights is defined using pop in 2030
...
write.csv(data.frame("year" = seq(2030, 2080, 10), "rights_proposed" = unname(t(round(v[v$region == "union", paste0("rights_proposed_", seq(2030, 2080, 10))])))), "../../NICE2020/cap_and_share/data/input/ffu_rights_proposed.csv", quote = F, row.names = F)
...
write.csv(data.frame("year" = seq(2030, 2080, 10), "rights_proposed" = unname(t(round(v[v$region == "union", paste0("rights_proposed_", seq(2030, 2080, 10))])))), "../data/ffu_rights_proposed.csv", quote = F, row.names = F)
...
df[df$code %in% c("DZA", "KAZ", "IRN", "IRQ", "LBY", "MNG", "ZAF", "TKM"), c("code", "region_tiam", "participate_union", "emissions_2025", "emissions_pc_2025")]
df$region_tiam[df$code %in% c("DZA", "KAZ", "IRN", "IRQ", "LBY", "MNG", "ZAF", "TKM")]
emissions_pc["2025"]*1.35
# Rights available for fossil countries:
(sum(v[v$region == "World", paste0("rights_", years_f)]) - sum(v[1:15, paste0("rights_proposed_", years_f)]))/1e9
# Pb: no more rights available in the first years
(sum(v[v$region == "union", paste0("emissions_bau_", 2030)]) - sum(v[v$region == "union", paste0("rights_proposed_", 2030)]))/1e9

v$emissions_bau_2030[v$region %in% "World"] - sum(v$emissions_bau_2030[v$region %in% c("CHI", "WEU")])
sum(df$emissions_2030[!df$region_tiam %in% c("CHI", "WEU")])
v$emissions_bau_2030[v$region %in% "World"]
sum(df$emissions_2030)
...
# temp <- read.csv(paste0("../../NICE2020/data/emissions_ndc_WEU_all.csv"), header = F)
# row.names(temp) <- 2020:2300
# colnames(temp) <- unique(nwco2$country)
# for (y in 2030:2080) for (c in df$code[df$region_tiam == "WEU"]) df[[paste0("rights_proposed_", y)]][df$code == c] <- temp[as.character(y), c]*1e9
...
# /!\ share_rights doesn't sum to 1 due to missing countries, hence sum(df$rights) < sum(df[, paste0("rights_", 2030:2080)]): only the latter is correct
...
# # df$rights_proposed[df$code %in% c("IRN", "LBY", "MNG", "TKM", "ZAF")] <- df$rights_proposed[df$code %in% c("IRN", "LBY", "MNG", "TKM", "ZAF")]*c(9.1, 8.88, 8.45, 10.51, 6.56)/4.86 # Scale rights to match current emissions at starting date
...
# additional_rights <- sum(df$rights_proposed[df$code %in% names(manual_adjust)]*(manual_adjust - 1))
# additional_rights <- sum(df[df$participate_union, paste0("rights_proposed_", 2030:2080)]) - sum(df[df$participate_union, paste0("rights_", 2030:2080)])
# additional_rights/1e9

# TODO! add CAN, AUS
# TODO! better map
# TODO! clean code, document
# TODO  change rights_proposed so that emissions decrease by > 2.5%/year
...
manual_adjust <- c("BHS" = 1.01, "BIH" = 1.1, "BWA" = 1.33, "EGY" = 1.07,"IDN" = 1.04, "IRN" = 1.22, "IRQ" = 1.07, "JOR" = 1.04, "LAO" = 1.33, "LBY" = 1.17, # LAO = 1.21, MNG = 1 suffice for var_npv_ede_ffu_nl > 0, but increased to lower transfers
                   "MNG" = 1.5, "MYS" = 1.61, "NAM" = 1.02, "PAN" = 1.6, "SUR" = 1.15, "THA" = 1.33, "TKM" = 1.63, "TUR" = 1.09) # 17 ctries, larger extra rights: MYS (+3.2 Gt), THA (2.8), IRN (2.1), IDN (1.1), TUR (.8), EGY (.8), TKM (.6), PAN (.4), IRQ (.3)
...
#                   "SUR" = 1.28, "SVK" = 1.16, "THA" = 1.18, "TKM" = 2.28, "TUR" = 1.18, "UKR" = 1.07) # SGP only country whose diff_npv_ede_ffu_nl remains < 0 (at -.0097)
# offset_EEU_partially <- c("AUT", "CHE", "DEU", "DNK", "ESP", "GBR", "FIN", "FRA", "HUN", "ITA", "ISL", "LVA", "LTU", "LUX", "NLD", "NOR", "SWE")
# manual_adjust <- c("BGR" = 1.07, "BIH" = 1.23, "BWA" = 1.3, "CHL" = 1.08, "CZE" = 1.2, 
#   "EST" = 1.25, "IRN" = 1.49, "IRQ" = 1.27, "JOR" = 1.01, "LAO" = 1.18, "LBY" = 1.25, 
...
# df$rights_proposed[df$code %in% offset_EEU_partially] <- df$rights_proposed[df$code %in% offset_EEU_partially]*.98 # Scale rights to prevent MICs from losing
# df$share_rights[df$code %in% offset_EEU_partially] <- df$share_rights[df$code %in% offset_EEU_partially]*.98
...
# setNames(nfede$diff_npv_ede_ffu_nl[nfede$time == 2030 & nfede$country %in% c("BHS", "BIH", "BWA", "EGY", "IDN", "IRN", "IRQ", "JOR", "LAO", "LBY", "MYS", "NAM", "PAN", "SUR", "THA", "TKM", "TUR")], c("BHS", "BIH", "BWA", "EGY", "IDN", "IRN", "IRQ", "JOR", "LAO", "LBY", "MYS", "NAM", "PAN", "SUR", "THA", "TKM", "TUR"))
...
# old: now replace by the line ... emissions_max[1]
# emissions_constraint_2030 <- if (spread == 0) 0 else ((v$emissions_bau_2030[v$region %in% "union"] - sum(df$rights_proposed_2030[df$region_tiam %in% c("CHI", "WEU")])) - (1 - spread)* sum(df$emissions_2030[df$participate_union & !df$region_tiam %in% c("CHI", "WEU")]))/spread
# df$rights_proposed_2030[!df$region_tiam %in% c("CHI", "WEU")] <- (spread * (df$share_rights/sum(df$share_rights[df$participate_union & !df$region_tiam %in% c("CHI", "WEU")])) * emissions_constraint_2030 + (1 - spread) * df$emissions_2030)[!df$region_tiam %in% c("CHI", "WEU")]
# df$rights_proposed_2030[!df$participate_union] <- v$rights[v$region == "World"]/df$pop_2030[!df$participate_union]
...
plot(2030:2080, colSums(df[df$participate_union, paste0("rights_proposed_", 2030:2080)])/1e9, type = 'l', ylim = c(0, 26))
lines(years_f, v[v$region == "union", paste0("rights_proposed_", years_f)]/1e9, type = 'l', lwd = 2)
lines(years_f, v[v$region == "union", paste0("emissions_bau_", years_f)]/1e9, type = 'l', col = "red")
lines(years_f, v[v$region == "union", paste0("rights_", years_f)]/1e9, type = 'l', col = "blue")
lines(2030:2080, colSums(df[df$region_tiam %in% "AFR", paste0("rights_proposed_", 2030:2080)])/1e9, type = 'l', col = "cyan")
lines(2030:2080, colSums(df[df$region_tiam %in% "IND", paste0("rights_proposed_", 2030:2080)])/1e9, type = 'l', col = "orange")
lines(2030:2080, colSums(df[df$region_tiam %in% "CHI", paste0("rights_proposed_", 2030:2080)])/1e9, type = 'l', col = "red")
lines(2030:2080, colSums(df[df$region_tiam %in% "ODA", paste0("rights_proposed_", 2030:2080)])/1e9, type = 'l', col = "blue")
lines(2030:2080, colSums(df[df$region_tiam %in% "WEU", paste0("rights_proposed_", 2030:2080)])/1e9, type = 'l', col = "green")

for (y in 2030:2080) df[[paste0("rights_proposed_pc_", y)]] <- df[[paste0("rights_proposed_", y)]]/df[[paste0("pop_", y)]]
rights_proposed_pc_union <- colSums(df[df$participate_union, paste0("rights_proposed_pc_", 2030:2080)] * df[df$participate_union, paste0("pop_", 2030:2080)])/colSums(df[df$participate_union, paste0("pop_", 2030:2080)])
# max(rights_proposed_pc_union[2:50]/rights_proposed_pc_union[1:49]) # -2.55%

# Using bau$emissions instead of v$bau shows that ODA, IND gain (and is consistent with the above computations, contrary to v), but for CSA it's the opposite
plot(2030:2080, rights_proposed_pc_union, type = 'l', ylim = c(0, 8))
lines(years_f, v[v$region == "union", paste0("emissions_bau_pc_", years_f)], type = 'l', col = "red", lty = 2)
# lines(years_f, v[v$region == "union", paste0("rights_proposed_pc_", years_f)], type = 'l', lwd = 2)
# lines(years_f, v[v$region == "union", paste0("rights_pc_", years_f)], type = 'l', col = "blue")
lines(2030:2080, colSums(df[df$region_tiam %in% "AFR", paste0("rights_proposed_pc_", 2030:2080)] * df[df$region_tiam %in% "AFR", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "AFR", paste0("pop_", 2030:2080)]), type = 'l', col = "cyan")
# lines(2030:2080, colSums(bau[df$region_tiam %in% "AFR", paste0("emissions_pc_", 2030:2080)] * df[df$region_tiam %in% "AFR", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "AFR", paste0("pop_", 2030:2080)]), type = 'l', lty = 3, col = "cyan")
lines(years_f, v[v$region == "AFR", paste0("emissions_bau_pc_", years_f)], type = 'l', lty = 2, col = "cyan")
lines(2030:2080, colSums(df[df$region_tiam %in% "IND", paste0("rights_proposed_pc_", 2030:2080)] * df[df$region_tiam %in% "IND", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "IND", paste0("pop_", 2030:2080)]), type = 'l', col = "orange")
# lines(2030:2080, colSums(bau[df$region_tiam %in% "IND", paste0("emissions_pc_", 2030:2080)] * df[df$region_tiam %in% "IND", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "IND", paste0("pop_", 2030:2080)]), type = 'l', lty = 3, col = "orange")
lines(years_f, v[v$region == "IND", paste0("emissions_bau_pc_", years_f)], type = 'l', lty = 2, col = "orange")
# lines(2030:2080, colSums(df[df$region_tiam %in% "CHI", paste0("rights_proposed_pc_", 2030:2080)] * df[df$region_tiam %in% "CHI", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "CHI", paste0("pop_", 2030:2080)]), type = 'l', col = "red")
# # lines(2030:2080, colSums(bau[df$region_tiam %in% "CHI", paste0("emissions_pc_", 2030:2080)] * df[df$region_tiam %in% "CHI", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "CHI", paste0("pop_", 2030:2080)]), type = 'l', lty = 3, col = "red")
# lines(years_f, v[v$region == "CHI", paste0("emissions_bau_pc_", years_f)], type = 'l', lty = 2, col = "red")
lines(2030:2080, colSums(df[df$region_tiam %in% "ODA", paste0("rights_proposed_pc_", 2030:2080)] * df[df$region_tiam %in% "ODA", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "ODA", paste0("pop_", 2030:2080)]), type = 'l', col = "blue")
# lines(2030:2080, colSums(bau[df$region_tiam %in% "ODA", paste0("emissions_pc_", 2030:2080)] * df[df$region_tiam %in% "ODA", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "ODA", paste0("pop_", 2030:2080)]), type = 'l', lty = 3, col = "blue")
lines(years_f, v[v$region == "ODA", paste0("emissions_bau_pc_", years_f)], type = 'l', lty = 2, col = "blue")
lines(2030:2080, colSums(df[df$code %in% "JPN", paste0("rights_proposed_pc_", 2030:2080)] * df[df$code %in% "JPN", paste0("pop_", 2030:2080)])/colSums(df[df$code %in% "JPN", paste0("pop_", 2030:2080)]), type = 'l', col = "green")
# lines(2030:2080, colSums(bau[df$code %in% "JPN", paste0("emissions_pc_", 2030:2080)] * df[df$code %in% "JPN", paste0("pop_", 2030:2080)])/colSums(df[df$code %in% "JPN", paste0("pop_", 2030:2080)]), type = 'l', lty = 3, col = "green")
lines(years_f, v[v$region == "JPN", paste0("emissions_bau_pc_", years_f)], type = 'l', lty = 2, col = "green")
# lines(2030:2080, colSums(df[df$region_tiam %in% "CSA", paste0("rights_proposed_pc_", 2030:2080)] * df[df$region_tiam %in% "CSA", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "CSA", paste0("pop_", 2030:2080)]), type = 'l', col = "blue")
# # lines(2030:2080, colSums(bau[df$region_tiam %in% "CSA", paste0("emissions_pc_", 2030:2080)] * df[df$region_tiam %in% "CSA", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "CSA", paste0("pop_", 2030:2080)]), type = 'l', lty = 3, col = "blue")
# lines(years_f, v[v$region == "CSA", paste0("emissions_bau_pc_", years_f)], type = 'l', lty = 2, col = "blue")
# lines(2030:2080, colSums(df[df$region_tiam %in% "WEU", paste0("rights_proposed_pc_", 2030:2080)] * df[df$region_tiam %in% "WEU", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "WEU", paste0("pop_", 2030:2080)]), type = 'l', col = "green")
# # lines(2030:2080, colSums(df[df$region_tiam %in% "WEU", paste0("emissions_pc_", 2030:2080)] * df[df$region_tiam %in% "WEU", paste0("pop_", 2030:2080)])/colSums(df[df$region_tiam %in% "WEU", paste0("pop_", 2030:2080)]), type = 'l', lty = 3, col = "green")
...
write.csv(df[, c("code", "region_tiam", "participate_union", paste0("rights_", 2030:2080))], "../../NICE2020/cap_and_share/data/input/equal_ffu_rights_allocation.csv", quote = F, row.names = F)
...
sum(df$rights_proposed[df$code %in% c("IRN", "LBY", "MNG", "TKM", "ZAF")])/1e9 
sum(df$rights_proposed[df$code %in% c("IRN", "LBY", "MNG", "TKM", "ZAF")]*df$emissions_pc_2025[df$code %in% c("IRN", "LBY", "MNG", "TKM", "ZAF")]/(sum(df$emissions_2025)/sum(df$pop_2025)))/1e9
sum(df$rights_proposed[df$code %in% c("IRN", "LBY", "MNG", "TKM", "ZAF")]*c(9.1, 8.88, 8.45, 10.51, 6.56)/4.86)/1e9 # using 2023 data from EDGAR fossil/indus CO2 https://en.wikipedia.org/wiki/List_of_countries_by_carbon_dioxide_emissions_per_capita, excluding KAZ because we believe it wouldn't join the Union
sum(df$rights_proposed[df$code %in% c("IRN", "IRQ", "TKM")])/1e9
sum(df$rights_proposed[df$code %in% c("IRN", "IRQ", "TKM")]*df$emissions_pc_2025[df$code %in% c("IRN", "IRQ", "TKM")]/(sum(df$emissions_2025)/sum(df$pop_2025)))/1e9
v$rights[v$region %in% "union"]

sum(df$rights_proposed_2030[df$region_tiam %in% c("WEU", "CHI")])
sum(v$emissions_bau_2030[v$region %in% c("WEU", "CHI")])
sum(df$rights_proposed_2030)/1e9
sum(df$emissions_2030)/1e9

sum(df$rights_proposed_2040[df$participate_union & !df$region_tiam %in% c("CHI", "WEU")])
(v$emissions_bau_2040[v$region %in% "union"] - sum(v$rights_proposed_2040[v$region %in% c("CHI", "WEU")]))
df$rights_proposed_2080[df$code == "IND"]
...
emissions_ffu <- read.csv("../../NICE2020/cap_and_share/output/revenue_recycling/global_per_capita/new_transfer/country_output/co2_emissions.csv")
...
nfco2$region <- region_tiam[nfco2$country]
for (r in unique(region_tiam)) carbon_budgets[r, "emissions_ffu"] <- round(sum(nfco2$E_gtco2[nfco2$time %between% c(2030, 2080) & nfco2$region %in% r], na.rm = T))
carbon_budgets

nftra <- read.csv(paste0(path_nice, "ffu_custom_transfers/country_output/transfer.csv"))
sum(nftra$transfer[nftra$time == 2040])

nbco2 <- read.csv("../../NICE2020/cap_and_share/output/bau_no_policy_at_all/no_revenue_recycling/country_output/co2_emissions.csv")
neco2 <- read.csv(paste0(path_nice, "equal_pc_transfer/country_output/co2_emissions.csv"))
ccco2 <- neco2 
ccco2[ccco2$time %between% c(2020, 2030),] <- nbco2[ccco2$time %between% c(2020, 2030),]
npop <- read.csv("../../NICE2020/cap_and_share/output/bau_no_policy_at_all/no_revenue_recycling/country_output/population.csv")
ccco2 <- merge(ccco2, npop)
ccco2$region <- nbco2$region <- neco2$region <- region_tiam[nfco2$country]

for (y in 2030:2050) ccco2$E_gtco2[ccco2$time == y] <- ccco2$l[ccco2$time == y] * (ccco2$E_gtco2[ccco2$time == 2020]/ccco2$l[ccco2$time == 2020] * (2050-y)/30 + ((y-2020)/30) * ccco2$E_gtco2[ccco2$time == 2050]/ccco2$l[ccco2$time == 2050])
for (r in unique(region_tiam)) carbon_budgets[r, "non_losing"] <- round(sum(ccco2$E_gtco2[ccco2$time %between% c(2030, 2080) & ccco2$region %in% r], na.rm = T))
for (r in unique(region_tiam)) carbon_budgets[r, "bau_nice"] <- round(sum(nbco2$E_gtco2[nbco2$time %between% c(2030, 2080) & nbco2$region %in% r], na.rm = T))
carbon_budgets[,c("rights_proposed", "emissions_ffu", "emissions_ndc", "emissions_cc", "non_losing", "bau_nice")]

nfede <- read.csv(paste0(path_nice, "ffu_custom_transfers/country_output/consumption_EDE.csv"))
nbede <- read.csv("../../NICE2020/cap_and_share/output/bau_no_policy_at_all/no_revenue_recycling/country_output/consumption_EDE.csv")
...
nfcpc$diff_cpc_ffu_bau <- nfcpc$CPC/nbcpc$CPC - 1

plot_map("diff_cpc_ffu_bau", nfcpc[nfcpc$time == 2040,], breaks = c(-Inf, -0.05, -0.025, -0.01, -0.0001, 0.0001, 0.01, 0.025, 0.05, 0.1, 0.3, Inf), labels = c("< -5", "-5 to -2.5", "-2.5 to -1","-1 to -0.1", "-0.1 to 0.1", "0.1 to 1", "1 to 2.5", "2.5 to 5", "5 to 10", "10 to 30", "> 30"), colors =c("#D05F68", "#E08A84", "#F7B7A1", "#FBE3D9", "#FFFFFF", "#E5F0FB", "#C0E0EF", "#92C5DE", "#6CAAD1", "#2166AC", "#103A71") )
temp <- nfcpc[nfcpc$time == 2125,]
temp$diff_cpc_ffu_bau_2025_2300 <- sapply(temp$country, function(c) sum(nfcpc$diff_cpc_ffu_bau[nfcpc$country %in% c & nfcpc$time %between% c(2025, 2300)]))
plot_map("diff_cpc_ffu_bau_2025_2300", temp, breaks = c(-Inf, -0.05, -0.025, -0.01, -0.0001, 0.0001, 0.01, 0.025, 0.05, 0.1, 0.3, Inf), labels = c("< -5", "-5 to -2.5", "-2.5 to -1","-1 to -0.1", "-0.1 to 0.1", "0.1 to 1", "1 to 2.5", "2.5 to 5", "5 to 10", "10 to 30", "> 30"), colors =c("#D05F68", "#E08A84", "#F7B7A1", "#FBE3D9", "#FFFFFF", "#E5F0FB", "#C0E0EF", "#92C5DE", "#6CAAD1", "#2166AC", "#103A71") )

nwcpc <- read.csv(paste0(path_nice, "../within_country/ffu_custom_transfers/country_output/consumption_per_capita.csv"))
nwcpc$diff_cpc_ffu_within <- nfcpc$CPC/nwcpc$CPC - 1
plot_map("diff_cpc_ffu_within", nwcpc[nwcpc$time == 2040,], breaks = c(-Inf, -0.05, -0.025, -0.01, -0.0001, 0.0001, 0.01, 0.025, 0.05, 0.1, 0.3, Inf), labels = c("< -5", "-5 to -2.5", "-2.5 to -1","-1 to -0.1", "-0.1 to 0.1", "0.1 to 1", "1 to 2.5", "2.5 to 5", "5 to 10", "10 to 30", "> 30"), colors =c("#D05F68", "#E08A84", "#F7B7A1", "#FBE3D9", "#FFFFFF", "#E5F0FB", "#C0E0EF", "#92C5DE", "#6CAAD1", "#2166AC", "#103A71") )

nwco2 <- read.csv(paste0(path_nice, "../within_country/ffu_custom_transfers/country_output/co2_emissions.csv"))
nwco2$region <- region_tiam[nwco2$country]
for (r in unique(region_tiam)) carbon_budgets[r, "ffu_within"] <- round(sum(nwco2$E_gtco2[nwco2$time %between% c(2030, 2080) & nwco2$region %in% r], na.rm = T))
carbon_budgets[,c("rights_proposed", "emissions_ffu", "emissions_ndc", "emissions_cc", "non_losing", "bau_nice", "ffu_within")]

# TODO: Run NICE separately for WEU / CHN / JPN / SKO to find emissions trajectories of their carbon budgets in the case without trade.
(budgets_ndc <- setNames((5.5*rowSums(v[, paste0("emissions_ndc_", c(2030, 2080))]) + 10*rowSums(v[, paste0("emissions_ndc_", seq(2040, 2070, 10))]))/1e9, v$region))
# EU budget of 11-14 GtCO2 (net all GHG) for 2030-2050: https://climate-advisory-board.europa.eu/reports-and-publications/scientific-advice-for-the-determination-of-an-eu-wide-2040/?utm_source=chatgpt.com

# TODO: Define non-losing rights from NDCs and CC
# TODO: Other method for non-losing trajectory: assume backstop price at net-zero target year and find the growth rate of carbon price required to meet the budget.
HIC <- df$code[df$gdp_pc_2019 > 13205] # approximately right (63 countries instead of 81 but differences are due to small islands)  
bau_nice_emissions <- read.csv("../../NICE2020/cap_and_share/output/bau_no_policy_at_all/no_revenue_recycling/country_output/co2_emissions.csv")
for (y in 2030:2100) for (c in intersect(df$code, bau_nice_emissions$country)) df[df$code == c, paste0("bau_nice_emissions_", y)] <- 1e9*bau_nice_emissions$E_gtco2[bau_nice_emissions$time == y & bau_nice_emissions$country == c]
for (y in 2030:2050) df[[paste0("non_losing_", y)]] <- df[[paste0("bau_nice_emissions_", y)]] * (2050 - y)/20 + (y - 2030)/20 * sum(df[[paste0("rights_", y)]])*df[[paste0("pop_", y)]]/sum(df[[paste0("pop_", y)]]) # df[[paste0("rights_", y)]] # Choosing pop at y or 2030 has negligible effect on MICs close to 0 net gain
for (y in 2030:2050) df[[paste0("non_losing_", y)]][is.na(df[[paste0("non_losing_", y)]])] <- df[[paste0("rights_", y)]][is.na(df[[paste0("non_losing_", y)]])] # SOM SSD TTO VEN PRK
for (y in 2051:2080) df[[paste0("non_losing_", y)]] <- pmax(0, df[[paste0("rights_", y)]])
for (y in 2081:2100) df[[paste0("non_losing_", y)]] <- 0
for (r in c("JPN", "KOR")) {
  temp <- read.csv(paste0("../../NICE2020/data/emissions_ndc_", r, ".csv"))
  names(temp) <- c("emissions_ndc", "year")
  for (y in 2030:2080) df[[paste0("non_losing_", y)]][df$code == r] <- temp$emissions_ndc[temp$year == y]*1e9
}
for (y in 2030:2080) df[[paste0("non_losing_", y)]][df$code == "CHN"] <- df[[paste0("rights_proposed_", y)]][df$code == "CHN"] # temp$emissions_ndc[temp$year == y]*1e9
temp <- read.csv(paste0("../../NICE2020/data/emissions_ndc_WEU_all.csv"), header = F)
row.names(temp) <- 2020:2300
colnames(temp) <- unique(nwco2$country)
for (y in 2030:2080) for (c in df$code[df$region_tiam == "WEU"]) df[[paste0("non_losing_", y)]][df$code == c] <- temp[as.character(y), c]*1e9
non_losing_file <- "../../NICE2020/cap_and_share/data/input/non_losing_rights.csv"
write.csv(df[, c("code", "region_tiam", "participate_union", paste0("non_losing_", 2030:2100))], non_losing_file, quote = F, row.names = F)
writeLines(gsub("non_losing_", "rights_proposed_", readLines(non_losing_file)), non_losing_file)

df$non_losing <- rowSums(df[,paste0("non_losing_", 2030:2080)])
df$rights_proposed_over_non_losing <- df$rights_proposed/df$non_losing - 1

df$non_losing[df$code %in% c("CHL", "CHN", "MYS", "ZAF")]/1e9
df$rights_proposed[df$code %in% c("CHL", "CHN", "MYS", "ZAF")]/1e9
(df[df$code %in% c("CHL", "MYS", "ZAF"), paste0("non_losing_", 2030:2080)] <= df[df$code %in% c("CHL", "MYS", "ZAF"), paste0("rights_proposed_", 2030:2080)])
sum(nfco2$E_gtco2[nfco2$country == "CHL" & nfco2$time %between% c(2030, 2080)])
sum(nfco2$E_gtco2[nfco2$country == "CHN" & nfco2$time %between% c(2030, 2080)])
sum(nfco2$E_gtco2[nfco2$country == "MYS" & nfco2$time %between% c(2030, 2080)])
sum(nfco2$E_gtco2[nfco2$country == "ZAF" & nfco2$time %between% c(2030, 2080)])
sum(df$rights_proposed)/1e9
sum(df$non_losing)/1e9
sum(df$rights_proposed[df$participate_union==T])/1e9
sum(df$non_losing[df$participate_union==T])/1e9
sum(df$rights_proposed[df$region_tiam == "AFR"])/1e9
sum(df$non_losing[df$region_tiam == "AFR"])/1e9
sum(df$non_losing[df$region_tiam == "IND"])/1e9
sum(bau_nice_emissions$E_gtco2[bau_nice_emissions$time %between% c(2030, 2080) & bau_nice_emissions$country %in% df$code[df$participate_union]])
sum(bau_nice_emissions$E_gtco2[bau_nice_emissions$time %between% c(2030, 2080) & bau_nice_emissions$country %in% df$code[df$region_tiam == "CHI"]])
df$rights_proposed_over_non_losing[df$code %in% c("CHN", "DZA", "IDN", "IRQ", "MEX", "VNM", "ZAF")]
sum((df$rights_proposed - df$non_losing)[df$code %in% c("CHN", "DZA", "IDN", "IRQ", "MEX", "VNM", "ZAF", "THA", "ARG", "MNG")])/1e9
# TODO: adjust rights_proposed for IRQ, PLN, THA, ZAF, CHN; using rights from AFR and those already adjusted e.g. MNG

(df$emissions_2019/df$footprint_2019)[df$code %in% c("CHN", "IRN", "LBY", "MNG", "SUR", "THA", "TKM", "UKR", "UZB", "ZAF")]
df$rights_proposed_over_non_losing[df$code %in% c("CHN", "IRN", "LBY", "MNG", "SUR", "THA", "TKM", "UKR", "UZB", "ZAF")]
# Increase IRQ, NAM, BWA, SUR, TKM, UKR, UZB; decrease LBY

temp <- read.csv("../data/footprint_over_territorial_2022.csv")
df <- merge(df, temp)
df$footprint_over_territorial_2022[is.na(df$footprint_over_territorial_2022)] <- 1

# TODO! Pb: rights_proposed should be > non-losing for acceptability; but then we don't need FFU since BAU=non-losing entail less emissions
...
plot_map("diff_ede_ffu_nl", nfede[nfede$time == 2050,], breaks = c(-Inf, -0.05, -0.025, -0.01, -0.0001, 0.0001, 0.01, 0.025, 0.05, 0.1, 0.3, Inf), labels = c("< -5", "-5 to -2.5", "-2.5 to -1","-1 to -0.1", "-0.1 to 0.1", "0.1 to 1", "1 to 2.5", "2.5 to 5", "5 to 10", "10 to 30", "> 30"), colors =c("#D05F68", "#E08A84", "#F7B7A1", "#FBE3D9", "#FFFFFF", "#E5F0FB", "#C0E0EF", "#92C5DE", "#6CAAD1", "#2166AC", "#103A71") )
plot_map("diff_ede_ffu_nl", nfede[nfede$time == 2060,], breaks = c(-Inf, -0.05, -0.025, -0.01, -0.0001, 0.0001, 0.01, 0.025, 0.05, 0.1, 0.3, Inf), labels = c("< -5", "-5 to -2.5", "-2.5 to -1","-1 to -0.1", "-0.1 to 0.1", "0.1 to 1", "1 to 2.5", "2.5 to 5", "5 to 10", "10 to 30", "> 30"), colors =c("#D05F68", "#E08A84", "#F7B7A1", "#FBE3D9", "#FFFFFF", "#E5F0FB", "#C0E0EF", "#92C5DE", "#6CAAD1", "#2166AC", "#103A71") )
...
# plot_map("diff_sum_ede_ffu_nl", nfede[nfede$time == 2030,], breaks = c(-Inf, -0.05, -0.025, -0.01, -0.0001, 0.0001, 0.01, 0.025, 0.05, 0.1, 0.3, Inf), labels = c("< -5", "-5 to -2.5", "-2.5 to -1","-1 to -0.1", "-0.1 to 0.1", "0.1 to 1", "1 to 2.5", "2.5 to 5", "5 to 10", "10 to 30", "> 30"), colors =c("#D05F68", "#E08A84", "#F7B7A1", "#FBE3D9", "#FFFFFF", "#E5F0FB", "#C0E0EF", "#92C5DE", "#6CAAD1", "#2166AC", "#103A71") )
# EXPORT 950 x 460
plot_world_map("var_npv_ede_ffu_nl", save = F, df[df$participate_union,], breaks = c(-Inf, 0, 0.003, 0.01, 0.03, 0.1, Inf), labels = c("< 0%", "0% to 0.3%", "0.3% to 1%", "1% to 3%", "3% to 10%", "> 10%"), colors = color(11)[c(1:5,8)], legend = "Welfare change:\nNet Present Value\nof Equally-Distributed\nEquivalent consumption\nFFU vs. non-losing\n(over 2030-2080, r = 3%)", na_label = "Non Parties", legend_x = .07)   
...
setNames(df$mean_transfer_over_gdp[df$code %in% c("IDN", "IRN", "IRQ", "JPN", "LAO", "MNG", "UKR")], c("IDN", "IRN", "IRQ", "JPN", "LAO", "MNG", "UKR"))
# EXPORT
plot_world_map("mean_transfer_over_gdp", save = F, df[df$participate_union,], breaks = c(-Inf, -0.05, -0.01, -0.003, 0.003, 0.01, 0.05, Inf), labels = c("< -5%", "-5% to -1%", "-1% to -0.3%", "-0.3% to 0.3%", "0.3% to 1%", "1% to 5%", "> 5%"), legend = "Net transfer\ndue to the\nFossil-Free Union\n(average over 2030-2080,\nin % of GDP)", legend_x = .075, na_label = "Non Parties", filename = "ffu_mean_transfer", format = "pdf")
...
setNames(df$rights_proposed_over_non_losing, df$code)[df$code %in% unique(nfede$country[nfede$diff_npv_ede_ffu_nl < 0])]
setNames(df$footprint_over_territorial_2022[df$code %in% c("BIH", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF")], c("BOS", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF"))
setNames(df$rights_proposed_over_non_losing[df$code %in% c("BIH", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF")], c("BOS", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF"))
setNames((df$rights_proposed - df$non_losing)[df$code %in% c("BIH", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF")]/1e9, c("BIH", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF"))
sum((df$rights_proposed - df$non_losing)[df$code %in% c("BIH", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF")])/1e9
sum((df$rights_proposed - df$non_losing)[df$code %in% c("BIH", "BWA", "IRN", "IRQ", "LBY", "MNG", "MYS", "SUR", "TKM", "TWN", "ZAF", unique(nfede$country[nfede$diff_npv_ede_ffu_nl < 0]))])/1e9
...
abline(h = seq(.5, 1.5, .5), lty = 9, col = "lightgrey")
axis(2, at = 1, labels = 1)
# lines(2030:2080, ffu_bi, type = 'l', col = "darkgreen", lwd = 2, lty = 3)
lines(2030:2080, ffu_revenue_pc_over_gdp, type = 'l', col = "darkgreen", lwd = 2, lty = 3)
...
round(sapply(v$region, function(r) sum(df[df$region_tiam == r, paste0("non_losing_", 2030:2080)]))/1e9)
sapply(v$region, function(r) sum(df$rights_proposed[df$region_tiam == r]))/1e9
sapply(v$region, function(r) sum(df$rights[df$region_tiam == r]))/1e9
sapply(v$region, function(r) sum(df$non_losing[df$region_tiam == r]))/1e9
sum(df[df$region_tiam %in% regions_union, paste0("rights_proposed_", 2030:2080)])
sum(df[df$region_tiam %in% regions_union, paste0("rights_", 2030:2080)])
sum(df[df$region_tiam %in% regions_union, paste0("non_losing_", 2030:2080)])
sum(df[df$code %in% "CHN", paste0("rights_proposed_", 2030:2080)])
sum(df[df$code %in% "CHN", paste0("rights_", 2030:2080)])
sum(df[df$code %in% "CHN", paste0("non_losing_", 2030:2080)])
sum(df[df$region_tiam %in% c("WEU", "EEU"), paste0("rights_", 2030:2080)])
sum(df[df$region_tiam %in% c("WEU", "EEU"), paste0("rights_proposed_", 2030:2080)])
sum(df[df$region_tiam %in% c("WEU", "EEU"), paste0("non_losing_", 2030:2080)])
sum(df$rights_proposed[df$region_tiam %in% c("WEU", "EEU")])
sum(df$rights[df$region_tiam %in% c("WEU", "EEU")])
sum(df[df$participate_union, paste0("non_losing_", 2030:2080)])
sum(df[df$participate_union, paste0("rights_proposed_", 2030:2080)])
sum(df[df$participate_union, paste0("rights_", 2030:2080)])
sum(df$rights_proposed[df$participate_union])
sum(df[, paste0("rights_proposed_", 2030:2080)])
...
(sum(df[,paste0("rights_proposed_", 2030:2080)])-sum(df$rights_proposed))/1e9
# Current trend Union:
sum(v$emissions_bau[v$region %in% regions_union]) + sum(df[df$participate_union & !df$region_tiam %in% regions_union, paste0("bau_nice_emissions_", 2030:2080)])*sum(v$emissions_bau[!v$region %in% c(regions_union, "World", "union")])/sum(df[!df$region_tiam %in% regions_union, paste0("bau_nice_emissions_", 2030:2080)]) # 941
emissions_bau_union_2030_2100 + sum(df[df$participate_union & !df$region_tiam %in% regions_union, paste0("bau_nice_emissions_", 2030:2100)])*sum(v$emissions_bau[!v$region %in% c(regions_union, "World", "union")])/sum(df[!df$region_tiam %in% regions_union, paste0("bau_nice_emissions_", 2030:2100)]) # 1154
...
sum(sort(setNames(df$rights_proposed[df$code %in% names(manual_adjust)]*(manual_adjust-1)/manual_adjust/1e9, names(manual_adjust))))
...
sum(df$emissions_2025[df$code %in% names(manual_adjust)])/sum(df$emissions_2025) # 8.5%
sum(df$pop_2025[df$code %in% names(manual_adjust)])/sum(df$pop_2025) # 6.3%
sum(df$rights[df$code %in% names(manual_adjust)])/1e9 
sum(df$rights)
# Pbs:
# >Transferts négatifs
# Emissions par pays / prix carbone identiques dans within_country et global_pc
# Changements de noms pas effectués dans tous les fichiers

# Q:
# pays_autorises?
# conso_EDE 2040 inférieure dans no_revenue_recycling vs. bau, alors qu'elle est > dans le papier de Marie => différentes starting year?
# Pourquoi "non-optimized"?
...
rowSums(df[df$code %in% c("AUS", "CAN", "USA"), paste0("rights_", 2030:2080)])/1e9
df$non_losing[df$code %in% c("AUS", "CAN", "USA")]/1e9

# EXPORT rights_proposed
temp <- read.csv(paste0("../../NICE2020/cap_and_share/data/input/E_Union_2020_2300.csv"))
sum(temp$E_gtco2[temp$time %between% c(2030, 2080)])
temp$E_gtco2[temp$time %between% c(2030, 2080)] <- pmax(0, colSums(df[df$participate_union, paste0("rights_proposed_", 2030:2080)])/1e9)
write.csv(temp, paste0("../../NICE2020/cap_and_share/data/input/E_ffu_2020_2300.csv"), quote = F, row.names = F)
