...
Let $g_i$ be country's $i$ net gain (or "equivalent transfer") in situation (transfers, prices) $j=(t_i, p_i)$ compared to the situation $c*=(0, p^*)$ (uniform within-country), in terms of equivalent variation (i.e. the transfer that would make it indifferent between the two situations). Define $V_i(p_i)$ the equivalent variation between situation $a=(0, p_i)$ and $c*$ (i.e. the compensation at $c*$ needed to get $a$'s utility):
...
Let $\pi^j_i$ the "equivalent price" such that $j=(t_i, p_i)$ is utility-equivalent to $j*=(0, \pi^j_i)$. <!-- correct u^{j*}_i -->
$$\begin{aligned}
u^j_i &= u^{j*}_i \\
...
u^j_i &= u_i(c_i + t_i, e^*_i(p_i))  = u^{0*}_i + (t_i - t^{0*}_i) \partial u_{i1} + p_i \partial u_{i2} \partial e^*_i \\
\Rightarrow t^{0*}_i &= t_i - p_i \frac{-\partial u_{i2} \partial e^*_i(0)}{\partial u_{i1}}
\end{aligned}$$
